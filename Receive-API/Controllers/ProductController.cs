...
namespace Receive_API.Controllers
{
    ...
    {   
        ...
        public async Task<IActionResult> GetAllProducts([FromQuery]PaginationParams param) {
            var data = await _serviceProduct.GetProducts(param);
            return Ok(data);
        }

        [HttpGet("get/{id}")]
        public async Task<IActionResult> GetProductByID(string id) {
            var data = await _serviceProduct.GetProduct(id);
            return Ok(data);
        }
